...
async def fetch_kline(stock_code: str, start_date: str = '', end_date: str = '', period: str = 'daily') -> pd.DataFrame:
    ...
    try:
        ...
        print(f'[data_source] akshare fetch_kline failed for {stock_code}: {e}')
        return pd.DataFrame()
    if df.empty:
        print(f'[data_source] akshare returned empty data for {stock_code}')
        return pd.DataFrame()
    # Cache to DB
    async with async_session_factory() as session:
...
async def search_stocks(query: str) -> list:
    try:
        ...
        print(f'[data_source] akshare search_stocks failed: {e}')
        return []
...
async def get_realtime(stock_code: str) -> dict:
    try:
        ...
        print(f'[data_source] akshare get_realtime failed for {stock_code}: {e}')
    return {}
